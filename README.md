...
#1. 데이터 분석 사고
핵심 질문 정의: 데이터 분석을 시작할 때 *"내가 이 데이터로 무엇을 알고 싶은가?"*에 대한 명확한 질문을 3개 이상 스스로 정의할 수 있어야 합니다.
...
이 두 가지가 무엇인지 정의하고, 어떤 기준과 원칙으로 처리(제거, 대체 등)했는지 설명할 수 있어야 합니다.

관찰과 해석의 구분: 그래프를 보고 눈에 보이는 객관적 사실을 말하는 '관찰(Fact)'과, 이를 바탕으로 이유를 유추하는 '해석/가설(Insight)'을 명확하게 구분하여 설명할 수 있어야 합니다.

2. 시계열 데이터 이해
시계열의 3대 개념:

트렌드(Trend): 시간에 따른 장기적인 방향성(상승/하강)

계절성(Seasonality): 일정한 주기를 두고 반복되는 패턴

노이즈(Noise): 불규칙하게 발생하는 무작위 변동

기본 분석 기법: 이동평균(노이즈를 줄이고 트렌드를 파악하기 위함)이나 변화율(전기 대비 증감 확인) 같은 기본 시계열 기법을 "왜" 쓰며, "어떻게" 적용했는지 설명할 수 있어야 합니다.

3. AI 활용 역량
효과적인 AI 활용 및 검증: 분석 목적에 맞는 명확한 질문을 AI에게 던지고, AI가 생성해 준 코드와 해석 결과가 올바른지 비판적으로 검증할 수 있어야 합니다.

주체적인 최종 판단: AI의 도움을 받더라도, 최종적인 해석과 결론은 반드시 본인의 논리와 근거를 들어 자신의 언어로 설명할 수 있어야 합니다.

정리해 드린 내용 중 가장 자신 있게 설명할 수 있는 부분은 어디이며,
